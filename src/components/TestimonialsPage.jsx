...
//     return <div className={`${className} !right-4 z-10`} onClick={onClick} style={{ display: "block" }} />;
//   }

//   function SamplePrevArrow(props) {
//     const { className, onClick } = props;
//     return <div className={`${className} !left-4 z-10`} onClick={onClick} style={{ display: "block" }} />;
//   }
...
const TestimonialsPage = () => {
  ...
  const settings = {
    ...
    customPaging: i => (
      <div className="w-3 h-3 bg-purple-500 rounded-full mx-1"></div>
    ),
  };

  function SampleNextArrow(props) {
    const { className, onClick } = props;
    return <div className={`${className} !right-4 z-10 text-purple-500`} onClick={onClick} style={{ display: "block" }} />;
  }

  function SamplePrevArrow(props) {
    const { className, onClick } = props;
    return <div className={`${className} !left-4 z-10 text-purple-500`} onClick={onClick} style={{ display: "block" }} />;
  }
    ...
    <div className="font-serif">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        <Slider {...settings}>
          {testimonials.map((item, idx) => (
            <div key={idx} className="px-4 h-full">
              <div className="bg-white border shadow-xl rounded-xl p-6 flex flex-col justify-between h-[340px]">
                <div className="text-yellow-400 text-xl mb-2">
                  {"★".repeat(item.stars) + "☆".repeat(5 - item.stars)} <span className="text-gray-600 ml-2">({item.stars}.0/5)</span>
                </div>
                <p className="italic text-gray-800 mb-4 line-clamp-5">
                  ❝{item.text}❞
                </p>
